    ...
    fetch(`/api/lavanda?action=dashboard&apiKey=${LAVANDA_KEY}`)
      ...
      .then(d => {
        if (d && d.kpis && d.daily && d.daily.length > 0) {
          ...
          console.warn("Lavanda live fetch returned unexpected shape, using static", d);
          setLavandaErr("Live fetch failed — showing snapshot");
        }
      })
      .catch(e => {
        console.warn("Lavanda live fetch failed, using static snapshot:", e.message);
        setLavandaErr("Live fetch failed — showing snapshot");
      })

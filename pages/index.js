...
export default function Dashboard() {
  ...
  const connectPMS = useCallback(async()=>{
    ...
    try{
      ...
      console.log("RH: blockedUnits=" + blockedUnits.length + ", sample units:", JSON.stringify(allUnits.slice(0,2)));
        ...
        {tab==="reputation"&&(
          <div style={{padding:"22px 26px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))",gap:12,marginBottom:16}}>
              {[
                {title:"Google My Business",rating:gmbRating,count:gmbCount,setRating:setGmbRating,setCount:setGmbCount,color:C.blue,icon:"G",url:"https://www.google.com/search?q=Southall+%26Soul"},
                {title:"Airbnb",rating:airbnbRating,count:airbnbCount,setRating:setAirbnbRating,setCount:setAirbnbCount,color:C.rose,icon:"A",url:"https://www.airbnb.co.uk/users/profile/1470683042353154223"},
                {title:"Trustpilot",rating:trustpilotRating,count:trustpilotCount,setRating:setTrustpilotRating,setCount:setTrustpilotCount,color:C.sage,icon:"T",url:"https://uk.trustpilot.com/review/andsoul.com"},
              ].map((p,i)=>(
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))",gap:12}}>
                <div style={{background:C.bg,borderRadius:10,padding:14,border:`1px solid ${C.rose}44`}}>
                  ...
                  <div style={{fontSize:12,color:C.muted,lineHeight:1.7}}>
                    <p style={{marginBottom:4}}>Volume is critically low — {trustpilotCount} reviews gives zero social proof. Aim for 50+ within 90 days.</p>
                    <p style={{marginBottom:4}}>Automate review requests — send Trustpilot invite links at check-out via RH/GHL automation.</p>
                    <p style={{marginBottom:4}}>Respond to every review — especially negatives. Thoughtful replies show accountability.</p>
                    <p>Incentivise with community points — offer cafe credit or event priority for honest reviews.</p>
                  </div>
                </div>
                <div style={{background:C.bg,borderRadius:10,padding:14,border:`1px solid ${C.gold}44`}}>
                  ...
                  <div style={{fontSize:12,color:C.muted,lineHeight:1.7}}>
                    <p style={{marginBottom:4}}>Address the recurring theme — air conditioning and ventilation issues appear in multiple reviews.</p>
                    <p style={{marginBottom:4}}>Improve first 24hr experience — welcome pack, room walkthrough, working amenities check.</p>
                    <p style={{marginBottom:4}}>Follow up on day 2 — a personal check-in message catches issues before they become bad reviews.</p>
                    <p>Superhost status — target 4.8+ overall. Resolve maintenance complaints to get there.</p>
                  </div>
                </div>
                <div style={{background:C.bg,borderRadius:10,padding:14,border:`1px solid ${C.sage}44`}}>
                  ...
                  <div style={{fontSize:12,color:C.muted,lineHeight:1.7}}>
                    <p style={{marginBottom:4}}>Target 100 reviews — you are at {gmbCount}, push to 100 for stronger local SEO.</p>
                    <p style={{marginBottom:4}}>Post weekly on GMB — share events, community photos. Google rewards active listings.</p>
                    <p style={{marginBottom:4}}>Add fresh photos monthly — listings with 100+ photos get 520% more calls.</p>
                    <p>Reply to all reviews within 24hrs — personalised responses, not ChatGPT templates.</p>
                  </div>
                </div>
                <div style={{background:C.bg,borderRadius:10,padding:14,border:`1px solid ${C.blue}44`,gridColumn:"1 / -1"}}>
                  ...
                  <div style={{fontSize:12,color:C.muted,lineHeight:1.7}}>
                    <p style={{marginBottom:4}}>Composite score of {reputationScore}/100 has significant room for improvement.</p>
                    <p style={{marginBottom:4}}>Fix operations first, then ask for reviews — recurring HVAC/maintenance complaints will sink any review campaign.</p>
                    <p style={{marginBottom:4}}>Implement a review funnel — happy guests get review link, unhappy guests get direct feedback channel.</p>
                    <p style={{marginBottom:4}}>Dedicated reputation owner — one team member monitors all 3 platforms daily, responds within 24hrs.</p>
                    <p>Target by Q3 2026 — Google 4.5+ (100 reviews), Airbnb 4.2+ (30 reviews), Trustpilot 4.0+ (50 reviews). Composite target: 75+.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:16}}>
              ...
              <textarea value={mentions} onChange={e=>setMentions(e.target.value)} placeholder="Paste mentions here..." style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,color:C.text,borderRadius:8,padding:"10px 12px",fontSize:12,fontFamily:"DM Mono,monospace",minHeight:80,boxSizing:"border-box",resize:"vertical"}}/>
              <div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
                <span style={{fontSize:10,background:mentions.toLowerCase().includes("love")||mentions.toLowerCase().includes("great")?"#3d9e7522":"#1c202855",color:mentions.toLowerCase().includes("love")||mentions.toLowerCase().includes("great")?C.sage:C.muted,padding:"4px 10px",borderRadius:12}}>
                  Positive: {mentions.split(" ").filter(w=>["love","great","amazing","best","excellent","perfect","friendly","community","recommend"].includes(w.toLowerCase().replace(/[^a-z]/g,""))).length}
                </span>
                <span style={{fontSize:10,background:mentions.toLowerCase().includes("issue")||mentions.toLowerCase().includes("problem")?"#c95c5422":"#1c202855",color:mentions.toLowerCase().includes("issue")||mentions.toLowerCase().includes("problem")?C.rose:C.muted,padding:"4px 10px",borderRadius:12}}>
                  Negative: {mentions.split(" ").filter(w=>["issue","problem","bad","awful","hate","disappointed","broken","dirty","sick","worst"].includes(w.toLowerCase().replace(/[^a-z]/g,""))).length}
                </span>
              </div>
            </div>
          </div>
